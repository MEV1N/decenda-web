...
export const challengeFileMap: Record<string, string | null> = {
    // OLD GARAGE
    ...
    'alley_calendar': '/challanges/cleander with circled date/token.py',
...
export default function Location() {
    ...
    const handleStartInstance = async () => {
        ...
        try {
            ...
            const msg = err.message || 'Failed to start instance.';
    ...
    for i in range(0, len(flag), 2)])`}</pre>
                                                ...
                                                {(() => {
                                                    const localUrl = activeChallenge.id in challengeFileMap ? challengeFileMap[activeChallenge.id] : activeChallenge.file_url;
                                                    if (activeChallenge.solved || activeChallenge.is_locked || !localUrl) return null;
                                                    ...
                                                    return (
                                                        <div className="mb-6 bg-zinc-900 border border-zinc-700 rounded p-4 shadow-inner">
                                                            ...
                                                            <div className="flex flex-wrap gap-2">
                                                                <button
                                                                    onClick={() => handleAuthenticatedDownload(localUrl)}
